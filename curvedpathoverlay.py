...
def curvedetection(frame, roi):
    ...
    if len(filconts) >= 2:
        ...
        poly1 = cv2.approxPolyDP(cont1, 10, closed=True)
        poly2 = cv2.approxPolyDP(cont2, 10, closed=True)

        poly1 = poly1[:int(len(poly1)*0.8)]
        poly2 = poly2[:int(len(poly1)*0.8)]

        cv2.polylines(frame, [poly1], isClosed=True, color=(0, 255, 0), thickness=2)
        cv2.polylines(frame, [poly2], isClosed=True, color=(0, 255, 0), thickness=2)
        ...
        centerline = cv2.approxPolyDP(centerline, epsilon=10, closed=True)
